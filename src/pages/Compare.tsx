...
const appetiteColors = {
  preferred: 'bg-green-900/50 text-green-300',
  standard: 'bg-blue-900/50 text-blue-300',
  limited: 'bg-amber-900/50 text-amber-300',
};
...
const quoteStatusColors: Record<string, string> = {
  quoted: 'text-success-500',
  pending: 'text-gray-500',
  referred: 'text-amber-400',
  ...
  if (!submissionId) {
    ...
    return (
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-100">Compare Quotes</h1>
        ...
        {quotable.length === 0 ? (
          <div className="text-center py-16 bg-gray-900 rounded-xl border border-gray-800">
            ...
            <p className="text-gray-400 text-lg font-medium">No submissions with quotes to compare</p>
            <p className="text-gray-500 text-sm mt-1">Submit new risks to carriers to start receiving quotes.</p>
          </div>
        ) : (
            ...
            {quotable.map(sub => {
              ...
              return (
                <button
                  ...
                  className="bg-gray-900 rounded-xl border border-gray-800 p-5 text-left hover:border-primary-700 hover:shadow-sm transition-all"
                >
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <h3 className="text-base font-semibold text-gray-100">{sub.business.name}</h3>
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-3">
                    {sub.requestedLines.map(line => (
                      <span key={line} className="text-xs px-2 py-0.5 rounded bg-gray-800 text-gray-400">
                        {line}
                      </span>
                    ))}
                  </div>
                  {lowestPremium < Infinity && (
                    <p className="text-sm text-gray-500">
                      From <span className="font-semibold text-gray-200">{formatCurrency(lowestPremium)}</span>/yr
                    </p>
                  )}
                </button>
              );
  ...
  if (!submission) {
    return (
      <div className="text-center py-16">
        ...
        <button onClick={() => navigate('/compare')} className="text-primary-400 mt-2 text-sm">
          Go back
        </button>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-2 text-xs text-primary-400 bg-primary-900/30 border border-primary-800 px-3 py-1.5 rounded-lg">
          ...
          <div className="space-y-2">
            {otherQuotes.map(quote => {
              ...
              return (
                <div
                  key={quote.id}
                  className="bg-gray-900 rounded-xl border border-gray-800 px-5 py-4 flex items-center gap-4"
                >
                  <StatusIcon className={`w-5 h-5 ${quoteStatusColors[quote.status]}`} />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-semibold text-gray-200">{quote.carrierName}</span>
                    ...
                    {quote.declineReason && (
                      <p className="text-xs text-red-400 mt-1">{quote.declineReason}</p>
                    )}
                    {quote.uwNotes && (
                      <p className="text-xs text-amber-400 mt-1">{quote.uwNotes}</p>
                    )}
                  </div>
      ...
      {filteredQuotes.length === 0 && (
        <div className="text-center py-16 bg-gray-900 rounded-xl border border-gray-800">
          ...
          <p className="text-gray-400 text-lg font-medium">No quotes for this line yet</p>
          <p className="text-gray-500 text-sm mt-1">Quotes will appear here as carriers respond.</p>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div
      className={`bg-gray-900 rounded-xl border-2 transition-all ${
        isSelected
          ? 'border-success-500 ring-2 ring-green-900'
          : quote.status === 'bound'
    ...
    >
      ...
      <div className="px-5 pt-4 pb-3">
        <div className="flex items-start justify-between mb-1">
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-base font-bold text-gray-100">{quote.carrierName}</h3>
              {isBestPrice && (
                <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-primary-900/50 text-primary-400">
                  Best Price
                </span>
              )}
              {quote.status === 'bound' && (
                <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-green-900/50 text-green-300">
                  Bound
                </span>
              )}
            </div>
        ...
        <div className="flex items-center gap-2 mt-0.5">
          ...
          {daysUntilExpiry !== null && (
            <span className={`text-xs font-medium ${daysUntilExpiry <= 5 ? 'text-danger-500' : 'text-gray-500'}`}>
      ...
      <div className="px-5 py-4 border-t border-gray-800">
        ...
        <div className="mt-2">
          ...
          <p className="text-sm font-semibold text-gray-300">{quote.coverageLimit}</p>
        </div>
      </div>
      ...
      <div className="border-t border-gray-800">
        <button
          ...
          className="w-full flex items-center justify-between px-5 py-2.5 text-xs text-gray-500 hover:bg-gray-800/50 transition-colors"
        >
