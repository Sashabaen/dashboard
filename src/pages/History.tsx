...
const statusConfig: Record<SubmissionStatus, { label: string; style: string; icon: typeof Shield }> = {
  draft: { label: 'Draft', style: 'bg-gray-800 text-gray-400', icon: FileText },
  submitted: { label: 'Submitted', style: 'bg-blue-900/50 text-blue-400', icon: FileText },
  quoting: { label: 'Quoting', style: 'bg-amber-900/50 text-amber-400', icon: Clock },
  quoted: { label: 'Quoted', style: 'bg-emerald-900/50 text-emerald-400', icon: CheckCircle2 },
  bound: { label: 'Bound', style: 'bg-green-900/50 text-green-300', icon: Shield },
  declined: { label: 'Declined', style: 'bg-red-900/50 text-red-400', icon: XCircle },
};
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        {(['submitted', 'quoting', 'quoted', 'bound', 'declined'] as SubmissionStatus[]).map(status => {
          ...
          return (
            ...
            >
              <Icon className={`w-4 h-4 mb-1 ${filterStatus === status ? 'text-primary-400' : 'text-gray-500'}`} />
              ...
              >
                ...
                >
                  ...
                  <div className="flex items-center gap-4 text-xs text-gray-500 shrink-0">
                    {quotedCount > 0 && (
                      <span className="text-gray-300 font-medium">{quotedCount} quote{quotedCount > 1 ? 's' : ''}</span>
                    )}
                  ...
                  <div className="border-t border-gray-800 px-5 py-4 space-y-4">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                      <div>
                        ...
                        <p className="font-medium text-gray-200">{new Date(sub.effectiveDate).toLocaleDateString()}</p>
                      </div>
                      <div>
                        <p className="text-xs text-gray-500">Employees</p>
                        <p className="font-medium text-gray-200">{sub.business.employeeCount}</p>
                      </div>
                      <div>
                        <p className="text-xs text-gray-500">Revenue</p>
                        <p className="font-medium text-gray-200">{formatCurrency(sub.business.annualRevenue)}</p>
                      </div>
                      <div>
                        <p className="text-xs text-gray-500">Submitted</p>
                        <p className="font-medium text-gray-200">{new Date(sub.createdAt).toLocaleDateString()}</p>
                      </div>
                    </div>

                    <div>
                      <p className="text-xs text-gray-500 mb-2">Requested Lines</p>
                      <div className="flex flex-wrap gap-1.5">
                        {sub.requestedLines.map(line => (
                          <span key={line} className="text-xs px-2 py-0.5 rounded-full bg-primary-900/40 text-primary-400 border border-primary-800">
                            {line}
                          </span>
                        ))}
                      </div>
                    ...
                    {sub.quotes.length > 0 && (
                      <div>
                        <p className="text-xs text-gray-500 mb-2">Quotes</p>
                        <div className="space-y-2">
                          {sub.quotes.map(q => (
                            <div key={q.id} className="flex items-center justify-between p-3 rounded-lg bg-gray-800">
                              <div className="flex items-center gap-3">
                                <span className={`w-2 h-2 rounded-full ${
                                  q.status === 'quoted' || q.status === 'bound' ? 'bg-success-500'
                                  : q.status === 'pending' ? 'bg-gray-500'
                                  : q.status === 'referred' ? 'bg-amber-400'
                                  : 'bg-red-400'
                                }`} />
                                <div>
                                  <p className="text-sm font-medium text-gray-200">{q.carrierName}</p>
